...
graficar.morosos <- function(df, indicador = "INDICADOR_MOROSO") {
  ...
  for (var in categoricas) {
    resumen <- df %>%
      ...
      summarise(
        total = n(),
        morosos = sum(.data[[indicador]] == 1, na.rm = TRUE),
        porcentaje_morosos = (morosos / total) * 100,
        .groups = "drop"
      ) %>%
      arrange(desc(porcentaje_morosos))
    
    print(resumen)  # muestra tabla resumen
    
    # Gráfico de barras con porcentaje de morosos
    p <- ggplot(resumen, aes(x = reorder(.data[[var]], -porcentaje_morosos),
                             y = porcentaje_morosos,
                             fill = .data[[var]])) +
      geom_bar(stat = "identity") +
